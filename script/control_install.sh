...
flush privileges;
EOF
execute_cmd "mysql -uroot < ~/keystone.sql"
...
EOF
execute_cmd "source ~/admin_openrc.sh"
...
EOF
execute_cmd "mysql -uroot < ~/glance.sql"
...
EOF
execute_cmd "mysql -uroot < ~/nova.sql"
...
flush privileges;
EOF
execute_cmd "mysql -uroot < ~/neutron.sql"
